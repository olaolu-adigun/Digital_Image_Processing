...
int trim (int v){
	// Truncate the value of v if less than 0
	if (v < 0){
	   return 0;
	}
	// Truncate the value of v if greater than 255
 	if (v > 255){
	   return 255;
	}	
	return v;
...
unsigned char*** FloydStein(unsigned char*** arr, int Size1, int Size) {
...
unsigned char *** Stuki(unsigned char*** arr2, int Size1, int Size){
	...
	for (int i = offset; i < Size1 + offset; i++){
		...
		err = F - G;
		arr[i][j][0] = char(G);
		arr[i][j+1][0] = char(trim(int(arr[i][j+1][0]) + int(round(err*(8.0/42.0)))));
		arr[i][j+2][0] = char(trim(int(arr[i][j+2][0]) + int(round(err*(4.0/42.0)))));

		arr[i+1][j+2][0] = char(trim(int(arr[i+1][j+2][0]) + int(round(err *(2.0/42.0)))));
		arr[i+1][j+1][0] = char(trim(int(arr[i+1][j+1][0]) + int(round(err *(4.0/42.0)))));
		arr[i+1][j][0]   = char(trim(int(arr[i+1][j][0])   + int(round(err *(8.0/42.0)))));
		arr[i+1][j-1][0] = char(trim(int(arr[i+1][j-1][0]) + int(round(err *(4.0/42.0)))));
		arr[i+1][j-2][0] = char(trim(int(arr[i+1][j-2][0]) + int(round(err *(2.0/42.0)))));

		arr[i+2][j+2][0] = char(trim(int(arr[i+2][j+2][0]) + int(round(err *(1.0/42.0)))));
		arr[i+2][j+1][0] = char(trim(int(arr[i+2][j+1][0]) + int(round(err *(2.0/42.0)))));
		arr[i+2][j][0]   = char(trim(int(arr[i+2][j][0])   + int(round(err *(4.0/42.0)))));
		arr[i+2][j-1][0] = char(trim(int(arr[i+2][j-1][0]) + int(round(err *(2.0/42.0)))));
		arr[i+2][j-2][0] = char(trim(int(arr[i+2][j-2][0]) + int(round(err *(1.0/42.0)))));
...

{
	...
	////// Stuki
	unsigned char ***arrSTK = Stuki(arr2D, Size1, Size);
	
	for (int i = 0; i < Size1; i++){
	    for (int j = 0; j < Size; j++){
		Imagedata1[i][j][0] = arrSTK[i+1][j+1][0];
	    }
	}
	file = fopen("Stuki.raw","wb");
